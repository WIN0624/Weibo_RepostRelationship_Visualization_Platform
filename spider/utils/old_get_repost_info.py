...
def get_repost_relationship(bw_id, repost_writer, logger):
    ...
    if que.empty():
        logger.error('Retried 5 times of bw {bw_id} but failed.')
    while not que.empty():
        bw_id, level = que.get()
        get_repost_info(center_bw_id, bw_id, level, repost_writer, logger)
...
def get_repost_info(center_bw_id, bw_id, level, writer, logger, since_date=None):
    if_crawl = True
    error = {}
    # 获取原博主信息
            ...
            try:
                ...
                if content.get('ok') == 1:
                    ...
                    for data in datas:
                        ...
                        # 将待爬取id放入队列，记录将其作为原博时所处level：为当前level+1
                        que.put([data['id'], level+1])
                        # 判断是否是规定时间之后产生的微博
                        if since_date:
