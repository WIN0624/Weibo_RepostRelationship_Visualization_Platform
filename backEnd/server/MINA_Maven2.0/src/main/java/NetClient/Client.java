    ...
    public rank_response_vs process(Request_query qRequest) {
        ...
        while((!haveConnect)&&(retryNum!=0)){
            try{
                ...
                // 此处修改server的ip
                String connectHost = "192.168.1.103";//hostList[(randomNumber+retryNum)%hostNumber];
...
//		connector.getFilterChain().addLast( "threadPool", new ExecutorFilter(Executors.newCachedThreadPool())); 
    ...
    public Request_query process_exp(String query,String page,String type ) {
        // 模拟用的
        Request_query rq = new Request_query();
	QueryAndPage queryAndPage=new QueryAndPage();
	queryAndPage.set_query(query);
	queryAndPage.set_page(page);
        rq.set_query_content(queryAndPage);
        rq.set_query_type(type);
//      rq.set_query("4496119730447079");
//       rq.set_query_type("bw_id");
//	rq.set_query("4478127273386078");
//	rq.set_query_type("rpBody");
    ...
    public static void main(String[] args) throws IOException{
        ...
        Request_query qrst = clt.process_exp(args[0],args[1],args[2]);
