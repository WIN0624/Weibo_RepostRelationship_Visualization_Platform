...
4. `get_topic.py`用于获取实时热搜主题以及相应热度，提供两种方式存储：csv, json.具体格式[index, topic, score].生成文件命名为 **爬取热搜时间.csv /.json.**如果要实现json形式输出，`fileFormatConversion.py`中使用的编码格式不能支持热搜主题转换为json，因此建议使用`get_topic.py`中的hot2json(),更为简便。
